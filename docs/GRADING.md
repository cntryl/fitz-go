...
| REQ-PROTO-010 | T0 | **PASS** | Routes remain opaque broker-owned values. The client performs shape-only validation for scheme, segment count, empty segments, and allowed wildcard placement; it does not normalize routes or validate existence, permissions, realm semantics, resource names, or auth claims. |
